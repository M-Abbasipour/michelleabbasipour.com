<?php
    include 'includes/header.php';
?>

    <div class="container-fluid page_wrapper">
        
      <div class="starter_template row">
            <div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2">
          
                    <h1 id="header_text">
                       <span class="magenta">m</span>ichelle <span class="magenta">a</span>bbasipour
                    </h1>
                <br>
                  <div class="col-sm-4 col-sm-offset-4 col-md-4 col-md-offset-4 pic">
                        <img id='featured_img' src="images/michelle.jpg">
                </div>
            </div>
        
        <!-- contact details --> 
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-4"><h4><span class="magenta">w:</span><a href='http://www.michelleabbasipour.com' target="_blank"> michelleabbasipour.com</a></h4></div>
            <div class="col-xs-12 col-sm-12 col-md-3"><h4><span class="magenta">t:</span> 07568 530324</h4></div>
            <div class="col-xs-12 col-sm-12 col-md-5"><h4><span class="magenta">e:</span> <a href='mailto:michelle2.abbasipour@live.uwe.ac.uk?Subject: enquiry from michelleabbasipour.com'> michelle2.abbasipour@live.uwe.ac.uk</a></h4></div>
        </div>
        
        <div class="row narrow_row">
            <div class="col-sm-12 col-md-12" id="border_adjustment"></div>
            <div class="col-sm-12 col-md-12"></div>
        </div>         

          
        <!-- main content -->
        <div class="row">
            
            <!-- sidebar -->
            <div class="col-md-4" id="sidebar">
                <h1 class='narrow magenta'>EDUCATION</h1>
                <h3 class='narrow'>BACHELORS OF SCIENCE</h3>
                <h4 class='magenta'>DIGITAL MEDIA</H4>
                <p class='emphasis'><a href='http://www.uwe.ac.uk' target="_blank">University of the West of England</a></p>
                <p class='emphasis'>From 2013 to 2016</p>
                <p class=''>Predicted: First</p>
                
                <hr>
            
                <h3 class='narrow'>FOUNDATION YEAR</h3>
                <h4 class='narrow magenta'>COMPUTING</H4>
                <p class='emphasis'><a href='http://www.uwe.ac.uk' target="_blank">University of the West of England</a></p>
                <p class='emphasis'>From 2012 to 2013</p>
                <p class=''>Results: First</p>
                
                <hr>
            
                <h3 class='narrow'>SECONDARY EDUCATION</h3>
                <h4 class='narrow magenta'>'A' LEVELS</H4>
                <p class='emphasis'><a href='http://http://www.redmaids.co.uk' target="_blank">The Red Maids' School</a></p>
                <p class='emphasis'>From 1994 to 1996</p>
                <p class=''>Economics: D</p>
                <p class=''>French: E</p>
                <p class=''>English Literature: D</p>
                
                 <hr>
            
                <h3 class='narrow'>SECONDARY EDUCATION</h3>
                <h4 class='narrow magenta'>GCSEs</H4>
                <p class='emphasis'><a href='http://http://www.redmaids.co.uk' target="_blank">The Red Maids' School</a></p>
                <p class='emphasis'>From 1989 to 1994</p>
                <p class=''>Art: A</p>
                <p class=''>Biology: C</p>
                <p class=''>Business Studies: B</p>
                <p class=''>Chemistry: B</p>
                <p class=''>English Language: B</p>
                <p class=''>English Literature: B</p>
                <p class=''>French: B</p>
                <p class=''>Geography: B</p>
                <p class=''>Maths: B</p>
                <p class=''>Physics: B</p>
                
                <h1 class=' magenta header1'>TECHNICAL SKILLS</h1>
                <p class=''>
                        HTML 5 
                        <span class="magenta">/</span>
                        CSS 3 
                        <span class="magenta">/</span>
                        PHP 
                        <span class="magenta">/</span>
                        Python
                        <span class="magenta">/</span>
                        Wordpress
                        <span class="magenta">/</span>
                        AJAX
                        <span class="magenta">/</span>
                        JavaScript
                        <span class="magenta">/</span>
                        SQL
                        <span class="magenta">/</span>
                        XML
                </p>
                
                <h1 class=' magenta header1'>INTERESTS &amp; HOBBIES</h1>
                <p class=''>
                        Knitting 
                        <span class="magenta">/</span>
                        Crochet
                        <span class="magenta">/</span>
                        My children 
                        <span class="magenta">/</span>
                        Scrapbooking
                        <span class="magenta">/</span>
                        Reading
                        <span class="magenta">/</span>
                        Coding
                        <span class="magenta">/</span>
                        World of Warcraft
                        <span class="magenta">/</span>
                        Movies and TV Shows
                        <span class="magenta">/</span>
                        Walking my dogs
                </p>    
                
                  
                
            </div>
            
            <!-- main details -->
            <div class="col-md-8" id="main_details">
                <h1 class='narrow magenta'>KEY SKILLS</h1>
                <h3 class='narrow'>PROGRAMMING</h3>
                <p>As part of my degree course, including the Foundation Year, I  have used many coding languages extensively including C, HTML5, CSS3. During the second year, I have also started using XML, PHP and JavaScript. 
Examples of code that I have written can be found on my repositories on GitHub and were all assignments for my degree. I achieved a First for all of these projects and further information on these can also be found on my website. During the third year, the majority of programming was for my main project and involved using PHP, HTML 5, CSS3, SQL and JavaScript. The Advanced Topics in Web Development module increases my knowledge and skills in these technologies, as well as using XML and understanding more theory about web services and architecture. </p><br>
                
                
                <h3 class='narrow'>PROBLEM SOLVING</h3>
                <p>As a programmer, I have a lot of experience of problem solving as a big part of coding is learning to solve puzzles and fix prob- lems. A recent example of when I have had to solve a problem is in the website I created for the Web Design Principles module in my Second Year. I applied a parallax effect for my background and used Firefox to design the site. However, when I tested the site using Chrome, the parallax effect did not work due to Chrome not supporting a vital attribute in the CSS. I researched this issue and found several reported solutions which I tried until one of them worked. As a result, the website now works in Chrome and Firefox, which can be viewed on my website.</p><br>
                
                <h3 class='narrow'>IDEA TO FINAL PRODUCT</h3>
                <p>In my first and second years I was given assignments which required creating a website from design to development. The majority of my third year was spent on a creative technologies project where I chose to research, design and develop a mobile salon management web application. This involved user research, testing and evaluation, creating wireframes, static and interactive mockups and then researching the technologies required to implement the design and functionality. These projects can be accessed at michelleabbasipour.com</p><br>
            
                <h1 class='narrow magenta'>WORK EXPERIENCE</h1>
                <h3 class='narrow'>CONSULTANT &amp; TEAM DEVELOPER <span class="magenta">/</span> <span class="grey"> <a href='http://www.slimmingworld.co.uk' target="_blank">SLIMMING WORLD</a></span></h3>
                <h4 class="magenta"> {November 2010 - July 2013}</h4>
                <p>Running my own groups as a Slimming World consultant, supporting over 100 members on a weekly basis. On becoming a Team Developer in November 2012, I also supported a team of six consultants who brought an additional ten groups and 600 members under my management.
Achieved a Ruby award for welcoming 40 new members in one night to an opening group in April 2011. Achieved a Diamond award for welcoming more than 60 new members to my group in January 2012. Was awarded a Silver award in January 2012 for maintaining over 100 members on each week for the month.</p><br>
                
                
                <h3 class='narrow'>RECRUITMENT ASSISTANT <span class="magenta">/</span> <span class="grey"><a href='http://www.nhsbt.nhs.uk' target="_blank">NHS BLOOD AND TRANSPLANT</a></span></h3>
                <h4 class="magenta"> {November 2010 - July 2013}</h4>
                <p>Working within a team to recruit staff for NHSBT which included processing applications and to organise interviews for centres all over England and North Wales, as well as other administra- tive duties.
Was given an “acting up” position for a year into a Senior Recruitment Assistant to cover maternity leave</p>
            
            </div>
        </div>
          
      </div>

    </div><!-- /.container -->

<?php
    include 'includes/footer.php';
?>